...
class FCCLIP(nn.Module):
    ...
    def filter_unknown_predictions(self, outputs, threshold=0.5):
        """只保留未知类别的预测结果"""
        # 获取预测的mask和对应的类别
        pred_masks = outputs["pred_masks"]
        pred_logits = outputs["pred_logits"]
        
        # 使用category_overlapping_mask过滤已知类别
        unknown_mask = ~self.category_overlapping_mask.to(pred_logits.device)
        
        # 对于每个预测,如果它属于未知类别则保留
        filtered_indices = []
        for i, logits in enumerate(pred_logits):
            max_score, pred_class = logits.max(dim=-1)
            if unknown_mask[pred_class] and max_score > threshold:
        ...
        return {
            ...
            "pred_logits": pred_logits[filtered_indices]
        }
